report 99126 "Post Payroll Salary Voucher T"
{
    ProcessingOnly = true;
    Caption = 'Post Temporary Payroll Salary Voucher';
    dataset
    {
        dataitem("Payroll PeriodXT"; "Payroll PeriodXT")
        {
            ...
            begin
                GetPeriodFilter := "Payroll PeriodXT".GetRangeMin("Payroll PeriodXT"."Starting Date");
                DateSpecified := "Payroll PeriodXT".GetRangeMin("Payroll PeriodXT"."Starting Date");
                if PayrollPeriod.Get(DateSpecified, "Payroll PeriodXT"."Responsibility Center") then begin
        ...
        dataitem("Employee Posting GroupX Temp"; "Employee Posting GroupX Temp")
        {
            ...
            dataitem(EarningsX; "EarningsX Temp-E")
            {
                ...
                begin
                    ...
                    PGMapping.Reset;
                    //PGMapping.SETRANGE(PGMapping.Code,"Employee Posting GroupX Temp".Code);
                    ...
                    if PGMapping.Find('-') then begin
                        ...
                        Error(Text000, "Employee Posting GroupX Temp".Code, EarningsX.Code, EarningsX.Description);
                    ...
                    PayrollHeader.SetRange("Responsibility Center", "Payroll PeriodXT"."Responsibility Center");
                    if PayrollHeader.FindFirst() then
                        GenJnline.Validate("Dimension Set ID", PayrollHeader."Dimension Set ID");
            ...
            dataitem(Employer; "DeductionsX Temp-E")
            {
                ...
                begin
                    ...
                    // if PGMapping.Get("Employee Posting GroupX Temp".Code, Employer.Code, PGMapping.Type::Deduction) then begin
                    //     GenJnline.Init;
                    //     Employer.CalcFields(Employer."Total Amount Employer");
                    //     GenJnline."Journal Template Name" := CompRec."Payroll Journal Template";
                    //     GenJnline."Journal Batch Name" := CompRec."Payroll Journal Batch";
            ...
            begin
                ...
                TotalNetPay := 0;
                Totalgross := 0;
                "Employee Posting GroupX Temp".TestField("Account No.");
                PayablesAcc := "Employee Posting GroupX Temp"."Account No.";
                LineNumber := LineNumber + 10;
            end;
        }
            ...
            begin
                ...
                if (DeductionsX."Total Amount" <> 0) then begin
                    ...
                    if PGMapping.FindFirst then begin
                        ...
                        Error(Text000, "Employee Posting GroupX Temp".Code, DeductionsX.Code, DeductionsX.Description);
        ...
        dataitem(LoansRec; "DeductionsX Temp-E")
        {
        ...
        dataitem(ExternalLoans; "DeductionsX Temp-E")
        {
            DataItemTableView = sorting(Code) where(Loan = const(true));
                        ...
                        if LoanApp.Find('+') then
                            if Loanproduct.Get(LoanApp."Loan Product Type") then
                                if Loanproduct.Internal = false then begin
                                    ...
                                    GenJnline."Line No." := GenJnline."Line No." + 10000;
                                    // GenJnline."Account Type":=GenJnline."Account Type"::"G/L Account";
                                    if PGMapping.Get("Employee Posting GroupX Temp".Code, DeductionsX.Code, 1) then begin
                                        GenJnline."Account Type" := PGMapping."Account Type";
                                        GenJnline."Account No." := PGMapping."G/L Account";
                                    end else
                                        Error(Text000, "Employee Posting GroupX Temp".Code, DeductionsX.Code, DeductionsX.Description);
        ...
        dataitem(Summary; "Integer")
        {
            ...
            begin
                ...
                // CasualsPG.SETRANGE("Employee PGroup","Employee Posting GroupX Temp".Code);
                // IF CasualsPG.FINDFIRST THEN BEGIN
                //    GenJnline."Account Type":=CasualsPG."Account Type";
                //    GenJnline."Account No.":=CasualsPG."Account No.";
                // END ELSE BEGIN
                GenJnline."Account Type" := "Employee Posting GroupX Temp"."Account Type";
                GenJnline."Account No." := "Employee Posting GroupX Temp"."Account No.";
                GenJnline.Validate("Account No.");
                //END;
            ...
            begin
                ...
                if Confirm('Would you like to preview the journals?', true) then begin
                    ...
                    Page.Run(page::"General Journal", GL);
                end;
            end;
        }
    }
    ...
    begin
        ...
        //Payperiodtext:="Payroll PeriodXT".GETFILTER("Payroll PeriodXT"."Starting Date");
        //Payperiodtext:=COPYSTR(Payperiodtext,4,6);
        //GetPeriodFilter:="Employee Posting GroupX Temp".GETRANGEMIN("Employee Posting GroupX Temp"."Pay Period Filter");
    ...
    var
        ...
        PostingGroup: Record "Employee Posting GroupX Temp";
        TaxAccount: Code[10];
        SalariesAcc: Code[10];
        PayablesAcc: Code[20];
        First: Code[10];
        ...
        EarningsCopy: Record "EarningsX Temp-E";
        LoanApp: Record "Loan Application";
        EmpAccMap: Record "Employee Account Mapping";
        PGMapping: Record "Temp Employee PGroups";
        ...
        AssignmentMatrix: Record "Assignment Matrix-X Temp-E";
    ...
    procedure GetPayPeriod(var PayPeriods: Record "Payroll PeriodXT")
    begin
